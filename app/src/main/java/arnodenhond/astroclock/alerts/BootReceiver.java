...
public class BootReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (pr.isRefreshLatLon()) {
			AstroClock.updateLocationData(context,pr); // Changed from getLocation
		}
		if (pr.isFirstrun()) {
			AstroClock.updateLocationData(context,pr); // Changed from getLocation
		...
		PendingIntent operation = PendingIntent.getBroadcast(context, 0, pintent, PendingIntent.FLAG_IMMUTABLE);
		am.set(AlarmManager.RTC_WAKEUP, nc.getNextMidDay(), operation);

		pintent = new Intent(PrefsReader.KEY_ALERT_MIDNIGHT);
		operation = PendingIntent.getBroadcast(context, 0, pintent, PendingIntent.FLAG_IMMUTABLE);
