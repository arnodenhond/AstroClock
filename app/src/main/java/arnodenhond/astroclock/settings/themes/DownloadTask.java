...
import java.io.ByteArrayOutputStream; // Replaced ByteArrayBuffer
...
// import org.apache.http.util.ByteArrayBuffer; // Removed
...
public class DownloadTask extends AsyncTask<Void, Integer, Void> {
	...
	private byte[] readResponse(InputStream inputStream) throws IOException {
		...
		ByteArrayOutputStream baos = new ByteArrayOutputStream(); // Changed from ByteArrayBuffer
		int current = 0;
		byte[] buf = new byte[8192];
		while ((current = bis.read(buf)) != -1) {
			baos.write(buf,0,current); // Changed from baf.append
		}
		bis.close();
		return baos.toByteArray(); // Changed from baf.toByteArray
	}
