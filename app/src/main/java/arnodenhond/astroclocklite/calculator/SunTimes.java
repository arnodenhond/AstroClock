 ...
 * Both take all the parameters necessary to make the calculation; that is,
 * nothing is assumed, and no system information is used. If the caller wants to
 * make, for example, the current year or geographical location the default,
 * then it has to do that itself. The caller also needs to supply the Sun's
 * zenith, which is subject to some interpretation. For most purposes, the
 * constant ZENITH defined in this class will do the trick for true sunrise and
 * sunset. There are other zenith constants for various types of twilight.
 * <p>
 * The methods in this class return a UTC time, not a local time. This allows
 * its operation to be independent of the system. If the caller requires a local
 * time, which is likely, it needs to convert it. Methods are provided in the
 * Java <code>TimeZone</code> class to do this, but they rely on the system time
 * zone and clock to be correct.
 * <p>
...
public class SunTimes {
	 ...
	 * we must convert it to UTC and then to a locale time. The result is
	...
	/**
	 * GetSunriseTimeUTC gets the time of sunrise at the specified latitude and
	 * longitude on the specified day. Result is returned in a Time object
	 * 
	 * <pre>
	 * year	   4-digit year
	 * month      month, 1-12
	...
	/**
	 * getSunsetTimeUTC gets the time of sunrise at the specified latitude and
	 * longitude on the specified day. Result is returned in a Time object
	 * 
	 * <pre>
	 * year	   4-digit year
