...
package arnodenhond.astroclocklite.calculator;

/**
 * Static methods of this class do the basic maths. Note you cannot instantiate
 * this class: it is like java.lang.Math.
 * <p>
 * The calculations follow closely the methods given in the book
 * "Practical Astronomy with your Calculator" by Peter Duffett-Smith 3rd Edition
 * Cambridge University Press (1988) ISBN 0 521 35699 7 (paperback) or ISBN 0
 * 521 35629 (hardback) The main modification to those methods being the uniform
 * use of radians in this code, instead of the degrees used in the book.
 */
public final class PhaseOfMoon {

	/**
	 * Private constructor prevents instantiation. This class is meant to
	 * provide only static methods. See langauge spec 8.1.2.1 (end)
	 */
	private PhaseOfMoon() {
	}

	/**
	 * The origin used for time. We actually work relative to 1990 Jan 0.0, ie 1
	 * Jan 1990 00:00:00 is treated as day *one* -- remembering 5 leap years as
	 * well
	 */
	...
	/**
	 * Solve Kepler's equation for orbit. Given the "mean anomaly" M (the angle
	 * measured from perigee if the orbit were a uniform circular one) and the
	 * eccentricity e, use Kepler's equation to solve for the true anomaly (the
	 * angle measured from perigee in the actual eliptical orbit).
	 * <p>
	 * 
	...
	private static double Kepler(double M, double e) {
		...
		nu = M + delta;
		// System.out.println ("Kepler: guess: nu " + nu + " (M=" + M + "; e=" +
		// e + ")");
	...
	/**
	 * Determine the phase of the moon at the given time. Given a time in
	 * milliseconds from 00:00:00 January 1st 1970, return the phase of the moon
	 * as an angle (in radians) where 0 represents the new moon, and pi
	 * represents the full moon (and of course 2*pi is back to the new moon).
	 */
	public static double MoonPhase(long time) {
		double N, // "Phase" of earth orbit round sun (0 - 2*pi)
		Msol, Ec, LambdaSol;
		double l, Mm, Ev, Ac, A3, Mmprime, A4, lprime, V, ldprime, D, Nm;
	...
	/**
	 * Find the time of the given phase nearest to the given time. Find the
	 * nearest time to "nearTime" at which the phase of the Moon is equal to
	 * "phase". The determination of "nearest" may be inexact if the actual
	 * phase at nearTime is very close to PI different from the required phase,
	 * as nearness is judged by the linear approximation rather than the full
