...
/**
 * 
 * Class for displaying offers and getting/spending Tap Points (virtual
 * currency).
 * 
 */
public class TJCOffers {
	...
	// ================================================================================
	/**
	 * Show available offers using a default userID of the device's unique
	...
	/**
	 * Show available offers using publisher provided userID, currencyID and
	 * currency selector flag. This method should only be used if the
	 * application supports multiple currencies. Apps that support multiple
	 * currencies cannot be managed by Tapjoy.
	 * 
	 * @param currencyID
			...
			if (nodeValue != null && nodeValue.equals("true")) {
				...
				if (points != null && name != null) {
					...
					try {
						...
						// TapjoyLog.i(TAPJOY_POINTS, "currencyName: " +
						// currencyName);
						// TapjoyLog.i(TAPJOY_POINTS, "pointsTotal: " +
						// pointsTotal);

						// If the notifier is valid, try to use it.
						if (tapjoyEarnedPointsNotifier != null) {
							// Is there valid data in the last saved local tap
							// points?
							if (lastLocalPointTotal != -9999) {
								// Did we get more points than what we last
								// recorded? Then call the notifier that points
								// have been earned.
