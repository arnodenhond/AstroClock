...
public class TapjoyFeaturedAppWebView extends Activity {
	...
	// private String clientPackage = ""; // Package of the application using
	// TapjoyConnect
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// setContentView(
		// getApplicationContext().getResources().getIdentifier("tapjoy_featured_app_web_view",
		// "layout", clientPackage));

		RelativeLayout layout = new RelativeLayout(this);

		// Load and display the Offers WebView.
		// webView = (WebView)
		// findViewById(this.getResources().getIdentifier("offersWebView", "id",
		// clientPackage));
		...
		// progressBar =
		// (ProgressBar)findViewById(getResources().getIdentifier("OfferProgressBar",
		// "id", clientPackage));
		...
		// TapjoyLog.i(TAPJOY_OFFERS, "load web url: " +
	...
	public void onConfigurationChanged(Configuration newConfig) {
		...
		if (webView != null) {
			// Calling javascript "window.onorientationchange();" does not work
			// here.
			// Delaying and then calling it does work so launch a task to do
			// this for us.
	...
	/**
	 * 
	 * This task will delay for a brief amount of time and then call javascript
	 * function "window.onorientationchange();" because calling this within the
	 * onConfigurationChanged method does not work correctly.
	 * 
	 */
	private class RefreshTask extends AsyncTask<Void, Void, Boolean> {
		...
		protected void onPostExecute(Boolean result) {
			if (webView != null) {
				// This javascript function will resize the page based on the
				// window dimensions.
	...
	/**
	 * This class handle all events of tapjoy customized view, like to open
	 * link, test and verify url then load it in browser or market depends upon
	 * the url
	 */
	private class TapjoyWebViewClient extends WebViewClient {
		...
		/**
		 * When any user hits any url from tapjoy custom view then this function
		 * is called before opening any user.
		 */
	...
	/**
	 * Allows access to the webview to show the offer wall (when Show More Apps
	 * is pressed on the full screen ad page).
	 */
	...
	/**
	 * Allows access to the webview to close this activity (when SKIP is pressed
	 * on full screen ad page).
	 */
