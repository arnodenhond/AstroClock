...
public class TapjoyConnectCore {
	...
	private static String deviceCountryCode = ""; // Default country code for
													// this device.
	private static String deviceLanguage = ""; // Default language for this
												// device.
	...
	private static String userID = ""; // Publisher defined userID for this
										// device.
	...
	private static String connectionType = ""; // Connection type (MOBILE or
												// WIFI)

	private static String secretKey = ""; // Secret key.
	private static String clientPackage = ""; // App package name.
	private static String referralURL = ""; // Referral parameter (format is
											// "referrer=com.package.name")

	private static String plugin = TapjoyConstants.TJC_PLUGIN_NATIVE; // Plugin
																		// type,
																		// by
																		// default
																		// this
	...
	private static boolean videoEnabled = false; // Whether videos are enabled
													// or not.
	private static boolean enableVideoCache = true; // Whether video caching is
													// enabled or not.
	private static String videoIDs = ""; // List of video IDs to pass to the
											// offer wall.
	...
	// PAID APP
	private static String paidAppActionID = null; // Pay-Per-Action ID for the
													// paid app.
	 ...
	 * @param app_ID
	 *            The Tapjoy APP ID for this app.
	 * @param secret_Key
	 *            The Tapjoy SECRET KEY for this app.
	 */
	...
	/**
	 * Constructs the generic URL parameters for the connect call. Includes the
	 * verifier/timestamp parameter. Does NOT include: publisher_user_id
	 * (publisher user ID) referrer (for referral tracking)
	 * 
	...
	/**
	 * Constructs the generic URL parameters. Does NOT include:
	 * publisher_user_id (publisher user ID) referrer (for referral tracking)
	 * verifier/timestamp
	 * 
	 * @return Generic URL parameters used in the connect call.
	 */
	...
	/**
	 * Constructs the generic URL parameters without an App ID. This is the base
	 * method for generating the URL parameters. Does NOT include: app_id (App
	 * ID) publisher_user_id (publisher user ID) referrer (for referral
	 * tracking) verifier/timestamp
	 * 
	...
	private static String getParamsWithoutAppID() {
		...
		if (macAddress != null && macAddress.length() > 0) {
			...
			// urlParams += TapjoyConstants.TJC_DEVICE_MAC_ADDRESS + "=" +
			// Uri.encode(macAddress) + "&";
	...
	/**
	 * Initialize data from the device information and application info. This
	 * data is used in our URL connection to the Tapjoy server.
	 */
	private void init() {
		...
		try {
			...
			try {
				...
				if (telephonyManager != null) {
					...
					// getNetworkOperator() returns MCC + MNC, so make sure it's
					// 5 or 6 digits total.
					// MCC is 3 digits
				...
				if (invalidDeviceID) {
					...
					// Do we already have an emulator device id stored for this
					// device?
					...
					// Otherwise generate a deviceID for emulator testing.
						...
						// Save the emulator device ID in the prefs so we can
						// reuse it.
			...
			try {
				// This is a backwards compatibility fix for Android 1.5 which
				// has no display metric API.
				// If this is 1.6 or higher, then load the class, otherwise the
				// class never loads and
				// no crash occurs.
	...
	/**
	 * Timer which executes the actionComplete with paid app actionID once
	 * completed.
	 */
	 ...
	 * @return TRUE if the response is handled and notifier has been called,
	 *         FALSE otherwise.
	 */
	private static boolean handleConnectResponse(String response) {
		...
		if (document != null) {
			...
			if (nodeValue != null && nodeValue.length() > 0) {
				...
				for (ApplicationInfo appInfo : applications) {
					...
					if ((appInfo.flags & ApplicationInfo.FLAG_SYSTEM) == 1) {
						// android.util.Log.i(TAPJOY, "system: " +
						// appInfo.packageName);
					}
					// User installed app.
					else {
			...
			if (nodeValue != null && nodeValue.equals("true")) {
				// TapjoyLog.i(TAPJOY_CONNECT,
				// "Successfully connected to tapjoy site.");
				return true;
			}
			// else
			// {
	...
	/**
	 * Releases the TapjoyConnectInstance object. This must be called whenever
	...
	public static String getConnectionType() {
		...
		// Adding this try/catch for weird nullPointerException issue which
		// rarely occurs on certain hardware ?
		try {
			// Get connection type.
	...
	/**
	 * Returns the verifier hash. The format is:
	...
	/**
	 * Returns the verifier hash for awardTapPoints The format is:
	...
	/**
	 * Sets the user ID for this device. This is used by the publisher to
	 * identify the user for their app. This needs to be called before any call
	 * to Offers, Full Screen Ad, etc. If this method is not called, the device
	 * ID will be used by default.
	 * 
	 * @param id
	 *            User ID which is used to identify this user for this device.
	 */
	public static void setUserID(String id) {
		...
		new Thread(new Runnable() {
			public void run() {
				...
				// The referralURL contains both referrer parameter and its
				// value e.g. referralURL="referrer=com.tapjoy.tapX"
	 ...
	 * 
	 * @param enabled
	 *            Whether videos are enabled or not.
	 */
	 ...
	 * 
	 * @param enable
	 *            TRUE to enable video caching, FALSE to disable video caching.
	 */
	...
	/**
	 * Sets the device ID for debugging purposes. This only works when the
	 * deviceID is invalid for this device (simulator/emulator).
	 * 
	 * @param id
	 *            String to use as the deviceID.
	 */
	 ...
	 * 
	 * @param total
	 *            Amount of tap points the user has.
	 */
	...
	/**
	 * Gets the tap points total from that was locally stored by
	 * saveTapPointsTotal. If -9999 is returned, then no data was previously
	 * stored.
	 * 
	...
	public void actionComplete(String actionID) {
		...
		// The PPA call is identical to Connect except the PPA ID is passed as
		// the app_id parameter.
	 ...
	 * {@link #requestTapjoyConnect(Context context, String appID, String secretKey)}
	 * .<br>
