...
public class TapjoyFeaturedApp {
	...
	private int displayCount = 5; // By default, each full screen ad is only
									// displayed five times before it no longer
									// being displayed.
	 ...
	 * 
	 * @param notifier
	 *            Class which receives callback with Full Screen Ad data.
	 */
	...
	public void getFeaturedApp(String theCurrencyID, TapjoyFeaturedAppNotifier notifier) {
		// Store the currency ID. We need to use use this if the offer wall is
		// launched from the webView.
	...
	public void showFeaturedAppFullScreenAd() {
		...
		if (fullscreenURL.length() != 0) {
			...
			// Add the currencyID so the correct currency appears on the offer
			// wall.
	 ...
	 * 
	 * @param count
	 *            Number of times an ad should appear.
	 */
	...
	/**
	 * Parses the response from the server and determines whether the connection
	 * was successful.
	 * 
	 * @param response
	 *            HTTP Response from the server.
	 * @return TRUE if the response has been handled and notified, FALSE
	 *         otherwise.
	 */
	private boolean buildResponse(String response) {
		...
		if (retValue) {
			...
			if (getDisplayCountForStoreID(featuredAppObject.storeID) < displayCount) {
				// Send the Full Screen Ad data to the callback and increment
				// the count for this ID.
				featuredAppNotifier.getFeaturedAppResponse(featuredAppObject);

				// Only increment if the store ID does not match this appID.
				// If they match, then this is a test offer so let that display
				// unlimited.
	...
	/**
	 * Returns the number of times an app has been displayed as a full screen ad
	 * ad.
	 * 
	 * @param storeID
	...
	/**
	 * Increments the number of times an app has been displayed as a full screen
	 * ad ad.
	 * 
	 * @param storeID
	 *            ID of the app to check.
