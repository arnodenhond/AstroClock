...
public class TapjoyVideoView extends Activity implements OnCompletionListener, OnErrorListener, OnPreparedListener {
	...
	private boolean clickRequestSuccess = false; // Whether the click request
													// was successful or not
													// (sent when video starts).
	private boolean allowBackKey = false; // Whether to allow the BACK key to
											// traverse to the previous
											// activity.
	private int timeRemaining = 0; // Time remaining on video for textview.
	private int seekTime = 0; // Time to seek to if there is a pause/resume on
								// the video.
	...
	private ImageView tapjoyImage; // Tapjoy watermark image shown on bottom
									// right while video is playing.
	...
	private void initVideoCompletionScreen() {
		...
		webView.setWebViewClient(new WebViewClient() {
			/**
			 * When any user hits any url from tapjoy custom view then this
			 * function is called before opening any user.
			 */
	...
	private void startVideo() {
		...
		// On Activity termination, if dialog was previously showing, don't
		// start the video yet.
		if (dialogShowing) {
			videoView.seekTo(seekTime);
		...
		// Init the video completion screen now so it'll be ready when the video
		// is done.
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if ((keyCode == KeyEvent.KEYCODE_BACK)) {
			// If the video is playing for the first time, don't allow
			// interruption.
	...
	protected Dialog onCreateDialog(int id) {
		...
		case DIALOG_WARNING_ID:
			dialog = new AlertDialog.Builder(this).setTitle("Cancel Video?").setMessage("Currency will not be awarded, are you sure you want to cancel the video?").setNegativeButton("End", new DialogInterface.OnClickListener() {
