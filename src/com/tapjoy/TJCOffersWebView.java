...
/**
 * This is customized Tapjoy web view. It initially shows list of applications
 * that user can download to earn points.
 */
public class TJCOffersWebView extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (extras != null) {
			...
			// Normal offers URL
				...
				if (TapjoyConnectCore.isVideoEnabled()) {
					// Is this device on wifi? Or are cached videos disabled?
					// Then show all videos.
		...
		// setContentView(
		// this.getResources().getIdentifier("tapjoy_offers_web_view", "layout",
		// clientPackage));
	...
	protected void onDestroy() {
		...
		if (webView != null) {
			// Potential issue on Android v1.6 where the webView resources do
			// not
			// automatically free themselves and can cause a signal 11 (SIGSEGV)
			// crash error.
	...
	private class TapjoyWebViewClient extends WebViewClient {
		...
		public void onPageFinished(WebView view, String url) {
			// TapjoyLog.i(TAPJOY_OFFERS, "onPageFinished : " +
			// System.currentTimeMillis());
		...
		public boolean shouldOverrideUrlLoading(WebView view, String url) {
			...
			// Open all other items in a new window.
			{
				...
				// Close this webview if we're not displaying the offerwall so
				// when
				// the market activity backs, it won't display a blank or
				// redirecting webview
				if (skipOfferWall) {
					finish();
				}
