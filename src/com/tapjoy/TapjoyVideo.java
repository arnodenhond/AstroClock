...
public class TapjoyVideo {
	...
	private Hashtable<String, TapjoyVideoObject> uncachedVideos; // Uncached
																	// videos.
	private Hashtable<String, TapjoyVideoObject> cachedVideos; // Cached videos.

	public TapjoyVideoObject videoToPlay;

	private static final String BACKGROUND_WATERMARK_NAME = "watermark"; // Tapjoy
																			// watermark.
	...
	/**
	 * Instance for this class so the TapjoyVideoView activity has access to
	 * this.
	 * 
	 ...
	 * 
	 * @param notifier
	 *            Class implementing TapjoyVideoNotifier.
	 */
	public void initVideoAd(TapjoyVideoNotifier notifier) {
		...
		// Videos can only play from external storage. Does not work from phone
		// internal storage.
		// cacheDir = Environment.getExternalStorageDirectory().toString() +
		// "/tapjoy/cache/" + TapjoyConnectCore.getClientPackage() + "/";
		...
		new Thread(new Runnable() {
			public void run() {
				...
				// urlParams += "&" + TapjoyConstants.TJC_CURRENCY_ID + "=" +
				// TapjoyConnectCore.getAppID();
				...
				if (returnValue) {
					...
					if (cachedFilesOnDisk != null) {
						...
						for (int i = 0; i < cachedFilesOnDisk.length; i++) {
							...
							// No matching key in uncached videos, delete the
							// file.
							else {
	...
	/**
	 * Handles the response to "get videos" by parsing XML and creating video
	 * data objects.
	 * 
	 * @param response
	 *            HTTP response of get videos.
	 * @return
	 */
	...
	public void loadNextVideo() {
		...
		// Load the next video if we're under the limit and the queue isn't
		// empty.
		...
		new Thread(new Runnable() {
			public void run() {
				...
				try {
					...
					// Directories must be created on SD card before file is
					// created.
					...
					// If for some reason the file length is zero, assume there
					// was a network error.
	...
	/**
	 * Set the video ids of cached videos so that they may be passed onto the
	 * offer wall.
	 */
